...
export function UsersTable() {
  ...
  const swrKey = useMemo(() => {
    ...
    params.set("limit", String(limit));
    return `/api/hr/admin/users?${params.toString()}`;
  ...
  const filterControls = (
    <div className="flex flex-wrap gap-2 items-end mb-2 w-full justify-between">
      ...
      <Button
        type="button"
        className="ml-auto hover:cursor-pointer bg-primary text-primary-foreground rounded-md px-4 py-2 text-sm font-medium shadow-xs hover:bg-primary/90 transition-colors flex items-center gap-2"
              ...
              : users?.map((user: UserWithDetails) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="px-4 py-3">
                      <UserActions
                        user={user}
                        onActionCompleteAction={handleActionComplete}
                      />
                    </TableCell>
                  </TableRow>
                ))}
